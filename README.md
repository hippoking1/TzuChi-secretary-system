...
### 4. 🗓️ 道場值班排班系統
* **雙道場排班矩陣**：支援宜蘭園區與東港聯絡處全月排班矩陣與各時段席位管理。
* **依規則自動排班系統 (全新功能 🤖)**：
  * **和氣週輪值體系**：支援園區四個和氣每週依序輪流值班，可自訂基準週起點與循環次序。
  * **整組星期輪班演算法**：內建和氣二園區女眾班週一至週日組別名冊，依月份出現之星期次序循環自動指派整組志工。
  * **超額備用輪替機制 (方案B)**：維持園區席位 4 位不變，組員人數 >4 人之組別自動啟用跨月跨次指標追蹤，多出人員標為「備用輪替」並於下次輪替時自動遞補。
  * **跨場地衝突檢測與園區優先調動**：自動交叉比對東港聯絡處當日排班，若發現同人重複排班，依原則**優先保留園區排班**，並提示調動東港值班名冊。
  * **覆蓋模式彈性選擇**：支援「⚡ 覆蓋全部席位」或「📝 僅填入空白未指派席位（保留已有排班）」。
  * **後台規則獨立管理介面**：提供 `/admin/duty-rules` 視覺化維護每週輪值順序與各星期組別名冊。
* **眾別智能過濾**：自動依男眾/女眾別過濾志工名單，快速完成每月輪值排班。
* **每日 08:30 LINE 綜合行程推播提醒**：Cloud Scheduler 每日定時向明日值班志工發送提醒推播。
